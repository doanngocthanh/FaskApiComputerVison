...
try:
    ...
    print("Warning: LoggingManager not available")
...
async def upload_model_step1(
    file: UploadFile = File(..., description="File model YOLO (.pt)"),
    model_name: str = Form(..., description="Tên model"),
    model_description: str = Form("", description="Mô tả model"),
    confidence_threshold: float = Form(0.5, description="Ngưỡng confidence mặc định")
):
    """Bước 1: Upload model YOLO và lưu tạm"""
...
async def test_sample_and_finalize_model(
    model_id: str,
    sample_image: UploadFile = File(..., description="Ảnh mẫu để auto-detect classes"),
    auto_generate_class_names: bool = Form(True, description="Tự động tạo tên class từ detection"),
    custom_class_names: str = Form("[]", description="Tên class tùy chỉnh (JSON array) - dùng nếu auto_generate_class_names=False")
):
    """Bước 2: Test với ảnh mẫu và tự động tạo class names hoặc dùng custom names"""
...
async def update_model_class_names(
    model_id: str,
    class_names: str = Form(..., description="JSON array với tên class mới"),
    update_sample_results: bool = Form(True, description="Có cập nhật lại sample results không")
):
    """Cập nhật tên các class của model đã upload"""
...
async def upload_yolo_model_legacy(
    file: UploadFile = File(..., description="File model YOLO (.pt)"),
    sample_image: UploadFile = File(..., description="Ảnh mẫu để test model"),
    model_name: str = Form(..., description="Tên model"),
    model_description: str = Form("", description="Mô tả model"),
    class_names: str = Form(..., description="JSON array tên các class"),
    confidence_threshold: float = Form(0.5, description="Ngưỡng confidence mặc định")
):
    """Upload model YOLO với ảnh mẫu (Legacy API - khuyên dùng 2-step process)"""
...
async def list_yolo_models():
    """Lấy danh sách tất cả YOLO models đã upload"""
    try:
...
async def detect_with_yolo_model(
    model_id: str,
    file: UploadFile = File(..., description="Ảnh để detect"),
    confidence_threshold: float = Form(0.5, description="Ngưỡng confidence")
):
    """Sử dụng YOLO model để detect objects trong ảnh"""
...
async def detect_and_ocr(
    model_id: str,
    file: UploadFile = File(..., description="Ảnh để detect và OCR"),
    languages: str = Form('["en","vi"]', description="JSON array các ngôn ngữ OCR"),
    confidence_threshold: float = Form(0.5, description="Ngưỡng confidence cho detection"),
    ocr_crop_padding: int = Form(5, description="Padding khi crop vùng để OCR"),
    return_cropped_images: bool = Form(False, description="Có trả về ảnh crop không")
):
    """Detect objects với YOLO rồi OCR các vùng phát hiện với EasyOCR"""
...
async def ocr_text_only(
    file: UploadFile = File(..., description="Ảnh để OCR"),
    languages: str = Form('["en","vi"]', description="JSON array các ngôn ngữ OCR"),
    return_bboxes: bool = Form(True, description="Có trả về bounding boxes không")
):
    """OCR thuần chỉ với EasyOCR, không detection"""
...
async def cleanup_cache():
    """Dọn dẹp cache models để giải phóng memory"""
    try:
